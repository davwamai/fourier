...
class Sin(Scene):
    def construct(self) -> None:
        ...
        m=ValueTracker(0)
        rate=2
        t.add_updater(lambda v,dt: v.increment_value(dt))
...
class Sin2(Scene):
    def construct(self):
        # Define a circle centered at 5 units to the left
        circle = Circle().shift(5*LEFT)

        # Define a vector at the center of the circle
        vector = Vector().shift(5*LEFT)

        # Define a time tracker
        time_tracker = ValueTracker(0)

        # The vector's angle updates with time
        vector.add_updater(lambda v: v.set_angle(time_tracker.get_value()))

        # Add the time tracker, circle, and vector to the scene
        self.add(time_tracker, circle, vector)
        
        # Define a line from the tip of the vector to the y-axis
        projection_line = Line(vector.get_end(), vector.get_end()+np.array([1-vector.get_end()[0]-4,0,0]))
        projection_line.add_updater(lambda v: v.become(Line(vector.get_end(), vector.get_end()+np.array([1-vector.get_end()[0]-4,0,0]))))
        self.add(projection_line)
        
        # Define an axes
        axes = NumberLine(x_range=[0,TAU+1,1])
        axes.next_to(projection_line, RIGHT, buff=0)
        self.add(axes)

        # Define a path that the tip of the vector will trace out
        path = VMobject(color=YELLOW)
        path.set_points_as_corners([projection_line.get_end(), projection_line.get_end()+UP*0.0001])

        # Define a path update function
        def path_update(mob, dt):
            line_to_end = Line(path.get_end(), projection_line.get_end())
            path.append_vectorized_mobject(line_to_end)
            path.shift(RIGHT*dt*0.5)
        ...
        explanation = MathTex(r"This vector is tracing out a cycle with frequency proportional to time.")
        explanation.to_edge(UP)
        self.add(explanation)

        # Wait for 20 seconds while the animation plays
        self.wait(20)
...
class SinCos(Scene):
    def construct(self) -> None:
        ...
        axes=NumberLine(x_range=[0,TAU+1,1])
        axes.next_to(line,RIGHT,buff=0)
        axes1 = NumberLine(x_range=[0, TAU, 1]).rotate(90*DEGREES)
